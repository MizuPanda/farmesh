...
export default function PostSupplyForm({ onClose, onSubmitted }: PostSupplyFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/listings', {
        ...
        body: JSON.stringify({ rawInput, product, quantity, unit, pricePerUnit, expirationDate }),
      });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border p-6" style={{ borderColor: "hsl(30 15% 88%)", backgroundColor: "hsl(40 33% 97%)" }}>
        ...
        {status === 'done' && (
          <p className="mt-3 text-sm text-green-700 font-medium">
            ✓ Listing posted! Matching Agent found <strong>{matchCount}</strong> match{matchCount !== 1 ? 'es' : ''}. Check your Matches tab.
          </p>
        )}
        {status === 'error' && (
      ...
      {/* AI Parsed Preview — teammate's styling, our live state values */}
